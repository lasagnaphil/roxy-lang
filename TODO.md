...
- [ ] AOT compilation to C — Phases 1–4 fully complete (codegen, runtime library + C++ wrappers, header generation, runtime unification across VM/AOT, `RoxyVM*` dropped from native signatures, AOT NativeRegistry dispatch with `aot_symbol_name` override + dual-mode `bind_native` + auto-emitted extern decls, `MapHeader` slimmed via per-VM dispatch side-table). Phase 5: function-level `#line` directives in generated AOT source landed; statement-level granularity is a follow-up. Other Phase 5 items (DCE, Relooper, `switch` lowering, readable variable names) deliberately not pursued — the C compiler's optimizer handles them and they don't affect debugger UX. See `docs/internals/c-backend.md`.
