...
Last updated: 2026-07-05 (the god-class split is **finished**: LifetimeChecker, TraitSystem, and GenericCallResolver extracted from SemanticAnalyzer, SemaContext introduced; lambda capture analysis deliberately stays inline — too coupled to the walkers to be worth a class boundary. FunctionContext bundling also landed: all per-function slots push/pop as one unit at every body entry point, which fixed the lambda branch-terminates leak and a lambda in-delete-destructor leak of the same class. The 8 verified bugs from the earlier deep review are fixed in commits `b7e9ab0`..`0fb1b44`.)
