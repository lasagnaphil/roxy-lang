...
- [ ] All-paths-return for lambdas: the all-paths-return diagnostic that landed with `when`-exhaustiveness (2026-07-13) covers free functions and methods but not lambda bodies (they analyze via `analyze_lambda_expr`, not `analyze_fun_body`). A lambda that falls off the end of a non-void body still returns a zero/default value (`end_function_body` → `emit_zero_value`) rather than erroring.
