...
- [ ] **Two return paths each containing a `uniq` struct literal that differ only in nil-vs-value for one variant field double-free the shared local**. Pattern: `if (cond) { return uniq T { ..., child = x }; } return uniq T { ..., child = nil };` where the *other* fields reference the same uniq local (say `parent = p`). The move analysis appears to double-consume `p` (or some other local moved by both struct literals), leading to `slab->states[slot_idx] == SlotState::ALIVE` in `slab_allocator.cpp:286` at program teardown. Workaround: hoist the nullable field to a var initialised to `nil`, assign it inside the `if`, and emit a single struct literal with a single return. Repro (fails): `fun Parser.parse_if_stmt(): uniq Stmt { … if (self.match1(Else)) { var else_branch = …; return uniq Stmt { if_condition = cond, if_then = then, if_else = else_branch }; } return uniq Stmt { if_condition = cond, if_then = then, if_else = nil }; }`. Consolidating to one return (with `var else_branch: uniq Stmt = nil` lifted above the `if`) compiles and runs cleanly.
