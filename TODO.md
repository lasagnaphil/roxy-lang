...
- [ ] **Abstract generic-fun instances from Phase B are not quarantined.** A bounded template body calling another generic function with a type-param-typed argument (`fun outer<T: Trait>(v: T) { identity(v); }`) instantiates `identity$$T` as a real pending fun instance, whose body is then analyzed outside the Phase B bounds context ("unknown type" noise) and offered to the IR builder. Mirror the `GenericStructInstance::is_abstract` quarantine (flag in `instantiate_fun`, skip in the pending-fun drains and IR-builder fun loops). The mangle marker already applies. Unprobed — verify with a test first.
