...
- [ ] `inout List<uniq T>` parameter is treated as a move across loop iterations — semantic analyzer rejects any call-site `self.m(inout xs)` that sits in a loop body with "variable X is moved in loop body without reassignment". Root cause: in `SemanticAnalyzer::check_call_args` the `if (param_types[i]->noncopyable()) consume_noncopyable(arg.expr, ...)` branch runs regardless of the arg modifier, so an `inout` noncopyable argument is treated identically to a by-value move. It should be suppressed when `arg.modifier == ParamModifier::Inout` (and `Out`). Repro (surfaced during Lox Phase 8): passing `inout List<uniq FunBody>` from `parse_program` down through `parse_declaration` in a `while (!is_at_end())` loop. Workaround in examples/lox: wrap the noncopyable list in a `uniq FunBodyBag { bodies: List<uniq FunBody> }` and thread it as `ref FunBodyBag` instead — `ref` doesn't trip the consume path, and the list can still be mutated through the ref.
