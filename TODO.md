...
Last updated: 2026-07-05 (LifetimeChecker, TraitSystem, and GenericCallResolver extracted from SemanticAnalyzer, SemaContext introduced — three cuts of the god-class split below; LambdaCaptureAnalyzer is the remaining candidate. Also fixed the lambda branch-terminates leak found during the LifetimeChecker extraction. The 8 verified bugs from the earlier deep review are fixed in commits `b7e9ab0`..`0fb1b44`.)
...
- [ ] **Split the god class — remaining candidate.** **LambdaCaptureAnalyzer** (~700 lines: contexts, `try_capture_identifier`, validate/synthesize/backfill, `ensure_self_captured_through`; hardest — it rewrites the AST and calls back into `analyze_stmt`, and `analyze_identifier_expr`/`analyze_this_expr` consult the context stack mid-walk). Takes `SemaContext&` like the landed extractions.
- [ ] **Bundle per-function context state.** Partially addressed: `m_move_states` + `m_branch_terminates` are bundled behind `LifetimeChecker::FunctionScope`, and `m_active_type_params/bounds` moved to GenericCallResolver (guarded inside `analyze_generic_template_body`). Still guarded ad hoc with `ScopedValue` at the analyzer's body-analysis entry points (`analyze_fun_decl`, `analyze_member_body`, `synthesize_lambda_call_fn`): `m_in_coroutine`, `m_coro_yield_type`, `m_in_delete_destructor`, `m_in_finally_depth`; the coroutine-method diagnostic gap (fixed 2026-07-05) was a forgotten setup at exactly one of them, and the lambda branch-terminates leak (fixed with the LifetimeChecker extraction) was the same error class. A single `FunctionContext` pushed/popped as a unit eliminates it.
