...
Last updated: 2026-07-05 (the god-class split is **finished**: LifetimeChecker, TraitSystem, and GenericCallResolver extracted from SemanticAnalyzer, SemaContext introduced; lambda capture analysis deliberately stays inline — too coupled to the walkers to be worth a class boundary. Also fixed the lambda branch-terminates leak found during the LifetimeChecker extraction. The 8 verified bugs from the earlier deep review are fixed in commits `b7e9ab0`..`0fb1b44`.)
