...
Last updated: 2026-07-05 (LifetimeChecker and TraitSystem extracted from SemanticAnalyzer, SemaContext introduced — first two cuts of the god-class split below; also fixed the lambda branch-terminates leak found during the LifetimeChecker extraction. The 8 verified bugs from the earlier deep review are fixed in commits `b7e9ab0`..`0fb1b44`.)
...
- [ ] **Split the god class — remaining candidates.** **GenericCallResolver** (~700 lines: unify/infer, both generic-call paths, template-ref coercion, bounds resolution/checking, Phase B type-param method lookup), **LambdaCaptureAnalyzer** (~700 lines: contexts, `try_capture_identifier`, validate/synthesize/backfill, `ensure_self_captured_through`; hardest — it rewrites the AST and calls back into `analyze_stmt`). Both take `SemaContext&` like the landed extractions.
- [ ] **Bundle per-function context state.** Partially addressed: `m_move_states` + `m_branch_terminates` are now bundled behind `LifetimeChecker::FunctionScope`. Still guarded ad hoc with `ScopedValue` at the four entry points (`analyze_fun_decl`, `analyze_member_body`, `synthesize_lambda_call_fn`, `analyze_generic_template_body`): `m_in_coroutine`, `m_coro_yield_type`, `m_in_delete_destructor`, `m_in_finally_depth`, `m_active_type_params/bounds`; the coroutine-method diagnostic gap (fixed 2026-07-05) was a forgotten setup at exactly one of them, and the lambda branch-terminates leak (fixed with the LifetimeChecker extraction) was the same error class. A single `FunctionContext` pushed/popped as a unit eliminates it.
