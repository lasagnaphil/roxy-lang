...
- Lists with bounds checking (`List<T>` with push/pop/len/cap methods; containers are move-only — they own a heap buffer, like `uniq` — with an explicit `.copy()` for an independent duplicate)
