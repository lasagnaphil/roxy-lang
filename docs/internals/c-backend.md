...
- [x] Emit `#line` directives mapping back to Roxy source. Function-level granularity: `IRFunction::source_line` is populated by the IR builder from each AST decl's `body->loc.line`; the C emitter writes `#line N "<source_path>"` at the start of each generated function body when `CEmitterConfig::source_path` is set. Statement-level granularity left for a follow-up.
...
Current test count: 76 C-backend E2E tests (12 Phase 1 + 15 Phase 2 + 31 Phase 3 + 2 Phase 4 step-3 tests covering the AOT main wrapper + 5 Phase 4 step-4 tests covering AOT NativeRegistry dispatch, AOT symbol override, and cross-TU linkage against extern decls + 3 Phase 5 tests covering `#line` directive emission), plus 5 unit tests for the runtime context API in `tests/unit/test_runtime_ctx.cpp`.
