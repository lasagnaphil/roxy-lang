...
enum class Opcode : u8 {
    ...
    // Element-address lvalues for out/inout container args (lifetimes.md "Container element
    // lvalues").
...
inline Opcode decode_opcode(u32 instr) { return static_cast<Opcode>((instr >> 24) & 0xFF); }

inline u8 decode_a(u32 instr) { return static_cast<u8>((instr >> 16) & 0xFF); }

inline u8 decode_b(u32 instr) { return static_cast<u8>((instr >> 8) & 0xFF); }

inline u8 decode_c(u32 instr) { return static_cast<u8>(instr & 0xFF); }

inline u16 decode_imm16(u32 instr) { return static_cast<u16>(instr & 0xFFFF); }

inline i16 decode_offset(u32 instr) { return static_cast<i16>(instr & 0xFFFF); }
...
struct BCFunction {
    ...
    u32 param_register_count; // Total registers needed for parameters (may exceed param_count for
                              // multi-register structs)
    ...
    Vector<BCStructFieldDelete>
        struct_field_deletes; // Field-cleanup actions for STRUCT descriptors (kinds 5/6)

    BCFunction()
        : param_count(0), param_register_count(0), register_count(0), local_stack_slots(0),
          ret_reg_count(1) {}
};
