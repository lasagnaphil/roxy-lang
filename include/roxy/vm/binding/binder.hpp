...
// Helper to call a function with arguments extracted from registers (u64*)
template<typename Ret, typename... Args, std::size_t... Is>
Ret call_with_indices(Ret(*fn)(Args...), u64* regs, u8 first_arg, std::index_sequence<Is...>) {
    return fn(RoxyType<Args>::from_reg(regs[first_arg + Is])...);
}

// Helper to call a void function
template<typename... Args, std::size_t... Is>
void call_void_with_indices(void(*fn)(Args...), u64* regs, u8 first_arg, std::index_sequence<Is...>) {
    fn(RoxyType<Args>::from_reg(regs[first_arg + Is])...);
}
...
struct FunctionBinder {
    ...
    static void invoke(RoxyVM* vm, u8 dst, u8 argc, u8 first_arg) {
        ...
        if constexpr (std::is_void_v<ReturnType>) {
            ...
            // Call function and convert result to u64
