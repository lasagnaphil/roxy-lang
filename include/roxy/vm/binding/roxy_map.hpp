...
// implementation.
template <typename K, typename V> using RoxyMap = roxy::Map<K, V>;

// RoxyType specialization for RoxyMap<K, V>
template <typename K, typename V> struct RoxyType<RoxyMap<K, V>> {
    static Type* get(TypeCache& tc) {
        return tc.map_type(RoxyType<K>::get(tc), RoxyType<V>::get(tc));
    }
    static RoxyMap<K, V> from_reg(u64 r) { return RoxyMap<K, V>(reinterpret_cast<void*>(r)); }
    static u64 to_reg(RoxyMap<K, V> map) { return reinterpret_cast<u64>(map.data()); }
};
