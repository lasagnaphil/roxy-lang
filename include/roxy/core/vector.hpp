...
template <typename T, typename Index = u32> class Vector {
...
public:
    ...
    Vector(Index size, const T& item)
        : m_capacity(size), m_size(size), m_data(size ? new T[size] : nullptr) {
    ...
    Vector(Vector&& other) noexcept { swap(*this, other); }
    ...
    Vector(std::initializer_list<T> lst)
        : m_capacity(lst.size()), m_size(lst.size()),
    ...
    // when the Vector is.
    void clear_keep_capacity() { m_size = 0; }
    ...
    template <class Predicate> T* find_if(Predicate&& predicate) {
        for (Index i = 0; i < m_size; i++) {
