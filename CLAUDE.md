...
> ⚠️ A coroutine parameter must be a plain one-register value: an `out`/`inout` param or a by-value struct param crashes at runtime, because the state struct round-trips every param through `SetField`/`GetField` sized from its declared type. See TODO.md → High Priority.
