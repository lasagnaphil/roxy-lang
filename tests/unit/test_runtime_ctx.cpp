...
TEST_SUITE("Runtime Context") {
    ...
    TEST_CASE("init installs the runtime's default allocator") {
        roxy_ctx ctx;
    ...
    TEST_CASE("roxy_rt_init swaps the default allocator to slab") {
    ...
    TEST_CASE("roxy_rt_init/shutdown are reference-counted") {
        REQUIRE(roxy_rt_default_allocator() == &roxy_malloc_allocator);
    ...
    TEST_CASE("roxy_alloc/free use the active allocator") {
        roxy_ctx ctx;
        roxy_ctx_init(&ctx);
    ...
    TEST_CASE("ScopedContext restores previous on scope exit") {
        roxy_set_ctx(nullptr);
    ...
    TEST_CASE("user_data is settable for embedder use") {
        int sentinel = 42;
