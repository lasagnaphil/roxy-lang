...
TEST_SUITE("StaticString") {
    ...
    TEST_CASE("construct from const char* with length") {
        StaticString<32> s("hello world", 5);
        CHECK(s.size() == 5);
        CHECK(strcmp(s.c_str(), "hello") == 0);
    }

    TEST_CASE("construct from StringView") {
    ...
    TEST_CASE("push_back to capacity") {
        StaticString<4> s;
        s.push_back('a');
        s.push_back('b');
        s.push_back('c');
    ...
    TEST_CASE("comparison between StaticStrings") {
    ...
    TEST_CASE("implicit StringView conversion") {
        StaticString<32> s("hello");
        StringView sv = s;
        CHECK(sv.size() == 5);
        CHECK(sv == "hello");
