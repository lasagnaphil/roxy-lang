...
TEST_SUITE("E2E Interop") {
    ...
    TEST_CASE("Bound integer functions") {
    ...
    TEST_CASE("Bound boolean functions") {
        BumpAllocator alloc(4096);
        TypeEnv type_env(alloc);
        NativeRegistry registry(alloc, type_env.types());
    ...
    TEST_CASE("Bound float functions") {
        BumpAllocator alloc(4096);
        TypeEnv type_env(alloc);
        NativeRegistry registry(alloc, type_env.types());
    ...
    TEST_CASE("Void return function") {
        BumpAllocator alloc(4096);
        TypeEnv type_env(alloc);
        NativeRegistry registry(alloc, type_env.types());
    ...
    TEST_CASE("Built-in natives via registry") {
        SUBCASE("print works") {
            const char* source = R"(
            fun test(): i32 {
                return 42;
    ...
    TEST_CASE("Mixed bound and built-in functions") {
        const char* source = R"(
        fun test(): i32 {
            var lst: List<i32> = List<i32>();
            lst.push(abs(-5));
    ...
    TEST_CASE("Native struct with manual method") {
        BumpAllocator alloc(8192);
        TypeEnv type_env(alloc);
        NativeRegistry registry(alloc, type_env.types());
    ...
    TEST_CASE("Native method with parameters") {
        BumpAllocator alloc(8192);
        TypeEnv type_env(alloc);
        NativeRegistry registry(alloc, type_env.types());
    ...
    TEST_CASE("Native method with multiple parameters") {
        BumpAllocator alloc(8192);
        TypeEnv type_env(alloc);
        NativeRegistry registry(alloc, type_env.types());
    ...
    TEST_CASE("Native struct with free function") {
        BumpAllocator alloc(8192);
        TypeEnv type_env(alloc);
        NativeRegistry registry(alloc, type_env.types());
    ...
    TEST_CASE("RoxyString: C++ compares strings") {
    ...
    TEST_CASE("RoxyString: C++ creates string for Roxy") {
        const char* source = R"(
        fun test(): i32 {
