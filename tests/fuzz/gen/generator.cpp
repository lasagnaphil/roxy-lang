...
struct GType {
    ...
    bool operator==(const GType& other) const { return kind == other.kind && index == other.index; }
    bool is_numeric() const { return kind == I32 || kind == I64 || kind == F32 || kind == F64; }
...
struct Generator {
    ...
    std::string type_name(GType type) const {
        switch (type.kind) {
            ...
            case GType::Str:
                return "string";
            case GType::StructT:
                return structs[type.index].name;
            case GType::EnumT:
                return enums[type.index].name;
        }
        return "i32";
    }
    ...
    std::vector<uint32_t> local_structs() const {
        ...
        for (uint32_t i = 0; i < structs.size(); i++) {
            if (structs[i].module_idx == cur_module)
                result.push_back(i);
        }
        return result;
    }
    ...
    std::vector<uint32_t> local_enums() const {
        ...
        for (uint32_t i = 0; i < enums.size(); i++) {
            if (enums[i].module_idx == cur_module)
                result.push_back(i);
        }
        return result;
    }
    ...
    GType random_var_type() {
        ...
        if (cfg.max_structs_per_module > 0 && ent.chance(20)) {
            ...
            if (!candidates.empty()) {
                return GType{GType::StructT,
                             candidates[ent.range(static_cast<uint32_t>(candidates.size()))]};
            }
        }
        if (cfg.max_enums_per_module > 0 && ent.chance(15)) {
            auto candidates = local_enums();
            if (!candidates.empty()) {
                return GType{GType::EnumT,
                             candidates[ent.range(static_cast<uint32_t>(candidates.size()))]};
            }
        }
    ...
    std::string literal(GType type, uint32_t depth) {
        switch (type.kind) {
            case GType::Bool:
                return ent.range(2) ? "true" : "false";
            case GType::I32:
                return std::to_string(ent.range(100));
            case GType::I64:
                return std::to_string(ent.range(1000)) + "l";
            case GType::F32:
            case GType::F64: {
                std::string text = std::to_string(ent.range(100));
                text += ".";
                text += std::to_string(ent.range(10));
                if (type.kind == GType::F32)
                    text += "f";
                return text;
            }
            case GType::Str:
            ...
            case GType::EnumT: {
                ...
                return enum_info.name + "::" +
                       enum_info
                           .variants[ent.range(static_cast<uint32_t>(enum_info.variants.size()))];
            }
            case GType::StructT:
    ...
    std::vector<size_t> assignable_vars_of(GType type) const {
        std::vector<size_t> result;
        for (size_t i = 0; i < scope.size(); i++) {
            if (scope[i].assignable && scope[i].type == type)
                result.push_back(i);
        }
        return result;
    }
    ...
    bool func_visible(size_t func_idx) const {
        ...
        if (func.receiver >= 0)
            return false; // methods dispatch via a receiver var
        if (func.module_idx == cur_module)
            return true;
        if (!func.is_pub)
            return false;
        for (uint32_t imported : imports) {
            if (imported == func.module_idx)
                return true;
        }
        return false;
    }
    ...
    std::string call_name(size_t func_idx) const {
        const FuncInfo& func = funcs[func_idx];
        if (func.module_idx == cur_module || func_from_imported(func_idx))
            return func.name;
    ...
    std::vector<size_t> void_calls() const {
        std::vector<size_t> result;
        for (size_t i = 0; i < cur_func && i < funcs.size(); i++) {
            if (!funcs[i].has_ret && func_visible(i) && affordable(funcs[i]))
                result.push_back(i);
        }
        return result;
    }
    ...
    std::vector<std::pair<size_t, size_t>> method_calls_returning(GType want) const {
        ...
        for (size_t v = 0; v < scope.size(); v++) {
            if (scope[v].type.kind != GType::StructT)
                continue;
    ...
    std::vector<std::pair<size_t, size_t>> fields_of_type(GType want) const {
        ...
        for (size_t v = 0; v < scope.size(); v++) {
            ...
            for (size_t f = 0; f < struct_info.fields.size(); f++) {
                if (struct_info.fields[f].type == want)
                    result.push_back({v, f});
            }
        }
        return result;
    ...
    std::vector<size_t> local_generic_fns() const {
        ...
        for (size_t i = 0; i < generic_fns.size(); i++) {
            if (generic_fns[i].module_idx == cur_module)
                result.push_back(i);
        }
        return result;
    }
    ...
    std::vector<size_t> local_generic_structs() const {
        ...
        for (size_t i = 0; i < generic_structs.size(); i++) {
            if (generic_structs[i].module_idx == cur_module)
                result.push_back(i);
        }
        return result;
    }
    ...
    std::string gen_expr(GType want, uint32_t depth) {
        ...
        if (want.is_numeric()) {
            ...
            if (want.kind != GType::Bool)
                options.push_back(Opt::Cast);
        ...
        if (have_methods)
            options.push_back(Opt::Method);
        if (have_fields)
            options.push_back(Opt::Field);
        if (have_generics)
            options.push_back(Opt::GenericCall);
        ...
        switch (options[ent.range(static_cast<uint32_t>(options.size()))]) {
            ...
            case Opt::Cast: {
                ...
                switch (want.kind) {
                    case GType::I32:
                        source = GType{GType::I64, 0};
                        break;
                    case GType::I64:
                        source = GType{GType::I32, 0};
                        break;
                    default:
                        source = GType{ent.range(2) ? GType::I32 : GType::I64, 0};
                        break;
                }
            ...
            case Opt::Call: {
                ...
                return gen_call(candidates[ent.range(static_cast<uint32_t>(candidates.size()))],
                                depth);
            }
            case Opt::Method: {
                auto candidates = method_calls_returning(want);
                auto [var_idx, func_idx] =
                    candidates[ent.range(static_cast<uint32_t>(candidates.size()))];
            ...
            case Opt::Field: {
                ...
                auto [var_idx, field_idx] =
                    candidates[ent.range(static_cast<uint32_t>(candidates.size()))];
                return scope[var_idx].name + "." +
                       structs[scope[var_idx].type.index].fields[field_idx].name;
            }
            case Opt::GenericCall: {
                auto candidates = local_generic_fns();
                const GenericFnInfo& generic =
                    generic_fns[candidates[ent.range(static_cast<uint32_t>(candidates.size()))]];
            ...
            case Opt::FString: {
                ...
                for (uint32_t i = 0; i < interp_count; i++) {
                    ...
                    if (prim.kind == GType::Str) {
                        ...
                        if (string_vars.empty())
                            prim = GType{GType::I32, 0};
                    }
                    if (prim.kind == GType::Str) {
                        auto string_vars = vars_of(prim);
                        interp =
                            scope[string_vars[ent.range(static_cast<uint32_t>(string_vars.size()))]]
                                .name;
                    } else {
    ...
    void gen_stmt() {
        ...
        if (!fields_of_assignable_structs().empty())
            options.push_back(Opt::FieldAssign);
        if (can_nest) {
            ...
            if (have_enum_var)
                options.push_back(Opt::When);
            if (cur_has_ret)
                options.push_back(Opt::EarlyReturn);
        }
        if (any_calls_exist())
            options.push_back(Opt::CallStmt);
        if (cfg.use_generics && !local_generic_structs().empty())
            options.push_back(Opt::GenericBox);
        ...
        switch (options[ent.range(static_cast<uint32_t>(options.size()))]) {
            ...
            case Opt::Assign: {
                ...
                for (size_t i = 0; i < scope.size(); i++) {
                    if (scope[i].assignable)
                        candidates.push_back(i);
                }
                const VarInfo& var =
                    scope[candidates[ent.range(static_cast<uint32_t>(candidates.size()))]];
            ...
            case Opt::When: {
                ...
                for (size_t i = 0; i < scope.size(); i++) {
                    if (scope[i].type.kind == GType::EnumT)
                        enum_vars.push_back(i);
                }
            ...
            case Opt::GenericBox: {
                ...
                const GenericStructInfo& generic = generic_structs[candidates[ent.range(
                ...
                line("var " + box_name + " = " + generic.name + " { value = " + gen_expr(inner, 1) +
                     " };");
    ...
    std::vector<std::pair<size_t, size_t>> fields_of_assignable_structs() const {
        std::vector<std::pair<size_t, size_t>> result;
        for (size_t v = 0; v < scope.size(); v++) {
            if (!scope[v].assignable || scope[v].type.kind != GType::StructT)
                continue;
            const StructInfo& struct_info = structs[scope[v].type.index];
            for (size_t f = 0; f < struct_info.fields.size(); f++) {
                result.push_back({v, f});
    ...
    void gen_main_fn() {
        ...
        for (uint32_t imported : imports) {
            ...
            for (uint32_t c = 0; c < call_count && checksum_budget > 0; c++) {
                gen_checksum_stmt(acc,
                                  pub_funcs[ent.range(static_cast<uint32_t>(pub_funcs.size()))]);
                checksum_budget--;
            }
        }
    ...
    void emit_module_body(bool is_main) {
        ...
        for (uint32_t imported : imports) {
            ...
            std::vector<size_t> pub_funcs;
            for (size_t i = 0; i < funcs.size(); i++) {
                if (funcs[i].module_idx == imported && funcs[i].is_pub)
                    pub_funcs.push_back(i);
            }
            if (pub_funcs.empty())
                continue;
            ...
            if (cfg.use_cross_module && i > 0) {
                if (is_main) {
                    ...
                    for (uint32_t j = 0; j + 1 < module_count; j++) {
                        if (!imported_by_someone[j] || ent.chance(10))
                            imports.push_back(j);
                    }
                } else {
                ...
                for (uint32_t imported : imports)
                    imported_by_someone[imported] = true;
            }
