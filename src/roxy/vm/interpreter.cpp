    ...
    while (vm->running) {
        ...
        switch (op) {
            ...
            case Opcode::CALL_EXTERNAL: {
                // Format: CALL_EXTERNAL dst, ext_func_idx, arg_count
                // External function calls to other modules
                u8 dst = a;
                u8 ext_func_idx = b;
                u8 arg_count = c;
                u8 first_arg = dst + 1;

                if (ext_func_idx >= vm->module->external_functions.size()) {
                    vm->error = "Invalid external function index";
                    return false;
                }

                const BCExternalFunction& ext_func = vm->module->external_functions[ext_func_idx];

                // For now, external functions must be resolved at link time
                // by the Compiler class. The resolved indices point to the
                // function in the current (linked) module.
                u32 resolved_func_idx = ext_func.resolved_func_idx;

                if (resolved_func_idx >= vm->module->functions.size()) {
                    vm->error = "External function not resolved - multi-module support not yet implemented";
                    return false;
                }
                ...
                // Check argument count
                if (arg_count != callee->param_count) {
                    vm->error = "Wrong number of arguments to external function";
                    return false;
                }
