...
static bool is_alpha_numeric(char c) { return is_alpha(c) || is_digit(c); }
...
Lexer::Lexer(const char* source, u32 length)
    : m_source(source), m_length(length), m_start(0), m_current(0), m_line(1), m_line_start(0) {
...
bool Lexer::is_at_end() const { return m_current >= m_length; }
...
TokenKind Lexer::check_keyword(u32 start, u32 len, const char* rest, TokenKind kind) {
    if (m_current - m_start == start + len && memcmp(m_source + m_start + start, rest, len) == 0) {
        return kind;
    }
...
TokenKind Lexer::identifier_type() {
    switch (m_source[m_start]) {
        case 'b':
            return check_keyword(1, 4, "reak", TokenKind::KwBreak);
        case 'c':
                    ...
                    case 'a':
                        if (m_current - m_start > 2) {
                            switch (m_source[m_start + 2]) {
                                case 's':
                                    return check_keyword(3, 1, "e", TokenKind::KwCase);
                                case 't':
                                    return check_keyword(3, 2, "ch", TokenKind::KwCatch);
                            }
                        }
                        break;
                    case 'o':
                        return check_keyword(2, 6, "ntinue", TokenKind::KwContinue);
                }
            }
            break;
        case 'd':
            return check_keyword(1, 5, "elete", TokenKind::KwDelete);
        case 'e':
            if (m_current - m_start > 1) {
                switch (m_source[m_start + 1]) {
                    case 'l':
                        return check_keyword(2, 2, "se", TokenKind::KwElse);
                    case 'n':
                        return check_keyword(2, 2, "um", TokenKind::KwEnum);
                }
            }
            break;
        case 'f':
            if (m_current - m_start > 1) {
                switch (m_source[m_start + 1]) {
                    case 'a':
                        return check_keyword(2, 3, "lse", TokenKind::KwFalse);
                    case 'i':
                        return check_keyword(2, 5, "nally", TokenKind::KwFinally);
                    case 'o':
                        return check_keyword(2, 1, "r", TokenKind::KwFor);
                    case 'r':
                        return check_keyword(2, 2, "om", TokenKind::KwFrom);
                    case 'u':
                        return check_keyword(2, 1, "n", TokenKind::KwFun);
                }
            }
            break;
        case 'i':
            if (m_current - m_start > 1) {
                switch (m_source[m_start + 1]) {
                    case 'f':
                        return check_keyword(2, 0, "", TokenKind::KwIf);
                    case 'm':
                        return check_keyword(2, 4, "port", TokenKind::KwImport);
                    case 'n':
                        return check_keyword(2, 3, "out", TokenKind::KwInout);
                }
            }
            break;
        case 'n':
            if (m_current - m_start > 1) {
                switch (m_source[m_start + 1]) {
                    case 'a':
                        return check_keyword(2, 4, "tive", TokenKind::KwNative);
                    case 'e':
                        return check_keyword(2, 1, "w", TokenKind::KwNew);
                    case 'i':
                        return check_keyword(2, 1, "l", TokenKind::KwNil);
                }
            }
            break;
        case 'o':
            return check_keyword(1, 2, "ut", TokenKind::KwOut);
        case 'p':
            return check_keyword(1, 2, "ub", TokenKind::KwPub);
        case 'r':
            if (m_current - m_start > 1) {
                switch (m_source[m_start + 1]) {
                    case 'e':
                        if (m_current - m_start > 2) {
                            switch (m_source[m_start + 2]) {
                                case 'f':
                                    return check_keyword(3, 0, "", TokenKind::KwRef);
                                case 't':
                                    return check_keyword(3, 3, "urn", TokenKind::KwReturn);
                            }
                        }
                        break;
                }
        ...
        case 's':
            if (m_current - m_start > 1) {
                switch (m_source[m_start + 1]) {
                    case 'e':
                        return check_keyword(2, 2, "lf", TokenKind::KwSelf);
                    case 't':
                        return check_keyword(2, 4, "ruct", TokenKind::KwStruct);
                    case 'u':
                        return check_keyword(2, 3, "per", TokenKind::KwSuper);
                }
            }
            break;
        case 't':
            if (m_current - m_start > 1) {
                switch (m_source[m_start + 1]) {
                    case 'h':
                        return check_keyword(2, 3, "row", TokenKind::KwThrow);
                    case 'r':
                        if (m_current - m_start > 2) {
                            switch (m_source[m_start + 2]) {
                                case 'u':
                                    return check_keyword(3, 1, "e", TokenKind::KwTrue);
                                case 'a':
                                    return check_keyword(3, 2, "it", TokenKind::KwTrait);
                                case 'y':
                                    return check_keyword(3, 0, "", TokenKind::KwTry);
                            }
                        }
                        break;
                }
            ...
            break;
        case 'u':
            return check_keyword(1, 3, "niq", TokenKind::KwUniq);
        case 'v':
            return check_keyword(1, 2, "ar", TokenKind::KwVar);
        case 'w':
            if (m_current - m_start > 1) {
                switch (m_source[m_start + 1]) {
                    case 'e':
                        return check_keyword(2, 2, "ak", TokenKind::KwWeak);
                    case 'h':
                        if (m_current - m_start > 2) {
                            switch (m_source[m_start + 2]) {
                                case 'e':
                                    return check_keyword(3, 1, "n", TokenKind::KwWhen);
                                case 'i':
                                    return check_keyword(3, 2, "le", TokenKind::KwWhile);
                            }
                        }
                        break;
                }
            ...
            break;
        case 'y':
            return check_keyword(1, 4, "ield", TokenKind::KwYield);
    }
    return TokenKind::Identifier;
}
...
Token Lexer::next_token() {
    ...
    switch (c) {
        ...
        case '>':
            if (match('>')) {
                return make_token(match('=') ? TokenKind::GreaterGreaterEqual
        ...
        case '|':
            ...
            if (match('='))
                return make_token(TokenKind::PipeEqual);
