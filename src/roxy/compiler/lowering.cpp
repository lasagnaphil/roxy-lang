...
BCFunction* BytecodeBuilder::build_function(IRFunction* ir_func) {
    ...
    for (const auto& ir_cleanup : ir_func->cleanup_info) {
        ...
        if (type->kind == TypeKind::Uniq) {
            ...
            record.elem_cleanup_idx = build_element_cleanup(type);
        } else if (type->is_map() && type->noncopyable()) {
            record.kind = 4;  // MAP_CLEANUP
            record.elem_cleanup_idx = build_element_cleanup(type);
        } else if (type->is_coroutine()) {
            // Coro<T>: by bytecode lowering time, generated_struct_type is populated.
            // Check if the struct has a destructor (generated by coroutine lowering).
            ...
            if (has_dtor) {
                ...
                char dtor_name_buf[256];
                snprintf(dtor_name_buf, sizeof(dtor_name_buf), "%.*s$$delete",
                         coro_struct->struct_info.name.size(),
                         coro_struct->struct_info.name.data());
                StringView dtor_name(dtor_name_buf,
                                     static_cast<u32>(strlen(dtor_name_buf)));
                auto dtor_it = m_func_indices.find(dtor_name);
                if (dtor_it != m_func_indices.end()) {
                    record.destructor_fn_idx = static_cast<u16>(dtor_it->second);
                }
...
u16 BytecodeBuilder::build_element_cleanup(Type* type) {
    BCElementCleanup desc;
    desc.kind = 0;
    desc.dtor_fn_idx = 0;
    desc.elem_cleanup_idx = 0xFFFF;
    desc.key_cleanup_idx = 0xFFFF;
    ...
    if (type->kind == TypeKind::Uniq) {
        ...
        u16 elem_idx = build_element_cleanup(type->list_info.element_type);
        desc.kind = 3;  // LIST
        desc.elem_cleanup_idx = elem_idx;
    } else if (type->is_map() && type->noncopyable()) {
        Type* key_type = type->map_info.key_type;
        Type* value_type = type->map_info.value_type;
        desc.kind = 4;  // MAP
        if (value_type && value_type->noncopyable()) {
            desc.elem_cleanup_idx = build_element_cleanup(value_type);
        }
        if (key_type && key_type->noncopyable()) {
            desc.key_cleanup_idx = build_element_cleanup(key_type);
        }
    }

    u16 index = static_cast<u16>(m_current_func->element_cleanups.size());
    m_current_func->element_cleanups.push_back(desc);
    return index;
}
