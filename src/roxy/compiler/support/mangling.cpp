...
StringView mangle_overload(BumpAllocator& alloc, StringView fun_name, Span<Type*> param_types) {
    ...
    pos += prefix.size();
    memcpy(buf + pos, fun_name.data(), fun_name.size());
    pos += fun_name.size();
    for (auto& pname : param_names) {
        buf[pos++] = '$';
        memcpy(buf + pos, pname.data(), pname.size());
        pos += pname.size();
    }
    buf[pos] = '\0';
...
StringView mangle_type_name(BumpAllocator& alloc, Type* type) {
    ...
    switch (type->kind) {
        ...
        case TypeKind::Struct:
            return type->struct_info.name;
        case TypeKind::Enum:
            return type->enum_info.name;
        case TypeKind::Trait:
            return type->trait_info.name;
        case TypeKind::Nil:
            return "nil";
        case TypeKind::Self:
            return "Self";
        ...
        case TypeKind::List: {
            ...
            pos += prefix.size();
            buf[pos++] = '$';
            memcpy(buf + pos, elem.data(), elem.size());
            pos += elem.size();
            buf[pos] = '\0';
            return StringView(buf, total_len);
        }
        case TypeKind::Map: {
            ...
            pos += prefix.size();
            buf[pos++] = '$';
            memcpy(buf + pos, key_name.data(), key_name.size());
            pos += key_name.size();
            buf[pos++] = '$';
            memcpy(buf + pos, val_name.data(), val_name.size());
            pos += val_name.size();
            buf[pos] = '\0';
            return StringView(buf, total_len);
        }
        case TypeKind::Coroutine: {
            ...
            pos += prefix.size();
            buf[pos++] = '$';
            memcpy(buf + pos, yield.data(), yield.size());
            pos += yield.size();
            buf[pos] = '\0';
            return StringView(buf, total_len);
        }
        ...
        case TypeKind::Weak: {
            ...
            pos += prefix.size();
            buf[pos++] = '$';
            memcpy(buf + pos, inner.data(), inner.size());
            pos += inner.size();
            buf[pos] = '\0';
            return StringView(buf, total_len);
        }
        case TypeKind::Function: {
            ...
            memcpy(buf + pos, ret_sep.data(), ret_sep.size());
            pos += ret_sep.size();
            buf[pos++] = '$';
            memcpy(buf + pos, ret_name.data(), ret_name.size());
            pos += ret_name.size();
            buf[pos] = '\0';
            return StringView(buf, total_len);
        }
