...
u32 BytecodeBuilder::compute_call_arg_reg_count(IRInst* inst, IRFunction* callee_func) const {
    ...
    for (u32 i = 0; i < num_args; i++) {
        ...
        bool param_is_ptr =
            (callee_func && i < callee_func->param_is_ptr.size() && callee_func->param_is_ptr[i]);
...
void BytecodeBuilder::reset_function_state(IRFunction* ir_func) {
    ...
    for (u32 i = 0; i < num_blocks; i++)
        m_block_offsets.push_back(NO_OFFSET);
...
void BytecodeBuilder::preallocate_registers(IRFunction* ir_func) {
    ...
    for (IRBlock* block : ir_func->blocks) {
        ...
        auto pre_alloc_target_params = [&](const JumpTarget& target) {
            if (!target.block.is_valid() || target.block.id >= ir_func->blocks.size())
                return;
...
void BytecodeBuilder::emit_prologue(IRFunction* ir_func) {
    ...
    for (u32 i = 0; i < ir_func->params.size(); i++) {
        ...
        if (m_registry && param.type && param.type->is_container()) {
            ...
            StringView copy_name = param.type->is_list() ? param.type->list_info.copy_native_name
        ...
        if (slot_count > 0 && slot_count <= 4) {
            ...
            emit_abc(Opcode::STRUCT_STORE_REGS, stack_ptr_reg, param_reg,
                     static_cast<u8>(slot_count));
            emit(0); // Padding word
    ...
    for (const auto& ir_handler : ir_func->exception_handlers) {
        ...
        if (!ir_handler.type_name.empty()) {
            ...
            if (type_it != m_type_indices.end()) {
                ...
                Type* exc_type =
                    m_type_env ? m_type_env->type_by_name(ir_handler.type_name) : nullptr;
                if (exc_type && exc_type->is_struct()) {
                    u16 type_idx = m_module->types.size();
                    u32 size_bytes = exc_type->struct_info.slot_count * 4;
                    m_module->types.push_back(
                        {ir_handler.type_name, size_bytes, exc_type->struct_info.slot_count});
        ...
        auto emit_range = [&](u32 first_block_id, u32 last_block_id) {
            ...
            if (after_block_id < ir_func->blocks.size()) {
                ...
                bc_handler.try_end_pc =
                    (after_offset != NO_OFFSET) ? after_offset : m_current_func->code.size();
            } else {
...
void BytecodeBuilder::build_cleanup_records(IRFunction* ir_func) {
    for (u32 ci_index = 0; ci_index < ir_func->cleanup_info.size(); ci_index++) {
        ...
        // the borrow's register isn't yet initialized (lifetimes.md "Counting mechanics" /
        // "Promotion").
        ...
        u32 ready_pc = ir_cleanup.value.id < m_value_ready_pcs.size()
                           ? m_value_ready_pcs[ir_cleanup.value.id]
                           : NO_READY_PC;
        if (ready_pc != NO_READY_PC && ready_pc > record.live_start_pc) {
            ...
            if (ready_pc >= record.scope_end_pc)
                continue;
            record.live_start_pc = ready_pc;
        }

        // Map SSA value to register
        if (!has_register(ir_cleanup.value))
            continue;
        ...
        if (ci_index >= m_cleanup_covered_blocks.size())
            continue;
        ...
        auto first_kill_pc_in = [&](u32 range_start, u32 range_end) -> u32 {
            ...
            for (u32 pc : kill_it->second) {
                if (pc >= range_start && pc <= range_end && pc < best)
                    best = pc;
            }
            return best;
        };
        ...
        for (u32 covered_block : covered) {
            ...
            u32 b_end =
                next_off != NO_OFFSET ? next_off : static_cast<u32>(m_current_func->code.size());
...
void BytecodeBuilder::allocate_multi_register_value(ValueId value, u32 reg_count) {
    ...
    for (u32 r = 1; r < reg_count; r++) {
        ...
        if (extra_reg != 0xFF)
            m_reg_to_value[extra_reg] = value.id;
    }
...
bool BytecodeBuilder::is_call_result(u32 value_id) const {
    if (!m_current_ir_func || value_id >= m_current_ir_func->values_by_id.size())
        return false;
    IRInst* def = m_current_ir_func->values_by_id[value_id];
    if (!def)
        return false;
    return def->op == IROp::Call || def->op == IROp::CallNative || def->op == IROp::CallExternal ||
           def->op == IROp::CallIndirect;
}

void BytecodeBuilder::reserve_call_window(IRInst* inst, u32 extra_regs_for_return,
                                          u32 total_arg_regs) {
    ...
    while (true) {
        ...
        if (m_has_spilling) {
            ...
            u32 scratch_top =
                static_cast<u32>(m_scratch_regs[0] > m_scratch_regs[1] ? m_scratch_regs[0]
                                                                       : m_scratch_regs[1]) +
                1;
            if (scratch_top > floor_reg)
                floor_reg = scratch_top;
        }
...
void BytecodeBuilder::spill_furthest() {
    ...
    auto take_furthest_spillable = [this](ActiveAlloc& out) -> bool {
        for (u32 i = m_active.size(); i > 0; i--) {
            ...
            if (val == NO_VALUE)
                continue;
            if (is_call_result(val))
                continue;
            Type* value_type = value_type_of(val);
            if (value_type && get_value_reg_count(value_type) > 1)
                continue;
            out = m_active[idx];
            for (u32 j = idx + 1; j < m_active.size(); j++)
...
u8 BytecodeBuilder::get_result_register(ValueId value) {
    ...
    if (m_spill_slots.count(value.id))
        return m_scratch_regs[0];
...
void BytecodeBuilder::compute_const_use_modes(IRFunction* ir_func) {
    ...
    for (u32 i = 0; i < num_values; i++)
        m_requires_register.push_back(false);

    auto mark_reg = [&](ValueId v) {
        if (v.is_valid() && v.id < num_values)
            m_requires_register[v.id] = true;
    };
...
void BytecodeBuilder::compute_cleanup_coverage(IRFunction* ir_func) {
    ...
    auto record_eligible = [](const IRCleanupInfo& info) {
        if (!info.value.is_valid() || !info.start_block.is_valid())
            return false;
        if (info.whole_function_scope)
            return false; // already spans [0, end)
        if (info.call_borrow)
            return false; // sub-block [RefInc, Nullify) window
        if (info.kind == IRCleanupKind::Unpin)
            return false;
        return true;
    };
            ...
            switch (inst->op) {
                ...
                case IROp::RefDec: {
                    if (!inst->unary.is_valid())
                        break;
                    if (m_cleanup_kill_pcs.find(inst->unary.id) == m_cleanup_kill_pcs.end())
                        break;
    ...
    auto handler_covered = [&](u32 h, u32 value_id) {
        ...
        while (!worklist.empty()) {
            u32 b = worklist.back();
            worklist.pop_back();
            if (block_has_kill(value_id, b))
                return true;
            if (ir_func->blocks[b]->terminator.kind == TerminatorKind::Return)
                saw_return = true;
    ...
    for (u32 record_index = 0; record_index < record_count; record_index++) {
        ...
        if (!record_eligible(info))
            continue;
        if (info.start_block.id >= num_blocks)
            continue;
        ...
        while (!pending.empty()) {
            ...
            visited[b] = true;
            covered.push_back(b);
            if (block_has_kill(info.value.id, b))
                continue;
            for_each_successor(b, [&](BlockId succ) {
                if (succ.is_valid())
                    pending.push_back(succ.id);
            });
...
void BytecodeBuilder::compute_liveness(IRFunction* ir_func) {
    ...
    for (IRBlock* block : ir_func->blocks) {
        ...
        for (IRInst* inst : block->instructions) {
            ...
            for_each_operand(inst,
                             [&](ValueId& operand) { mark_use(m_live_ranges, operand, point); });
            point++;
        }
        ...
        for_each_terminator_operand(
            term, [&](ValueId& operand) { mark_use(m_live_ranges, operand, terminator_point); });
    ...
    for (u32 block_index = 0; block_index < ir_func->blocks.size(); block_index++) {
        ...
        auto extend_for_back_edge = [&](BlockId target_id) {
            if (!target_id.is_valid() || target_id.id >= ir_func->blocks.size())
                return;
            if (target_id.id >= block_index)
                return; // Not a back edge
            ...
            for (u32 value_index = 0; value_index < num_values; value_index++) {
                ...
                if (live_range.def_point < loop_start && live_range.last_use_point >= loop_start &&
    ...
    for (const auto& cleanup_record : ir_func->cleanup_info) {
        if (!cleanup_record.value.is_valid() || cleanup_record.value.id >= num_values)
            continue;
        u32 scope_end_point;
        if (cleanup_record.whole_function_scope) {
            ...
            if (!cleanup_record.end_block.is_valid() ||
                cleanup_record.end_block.id >= block_points.size())
                continue;
    ...
    for (u32 value_index = 0; value_index < num_values; value_index++) {
        ...
        while (lo < hi) {
            u32 mid = lo + (hi - lo) / 2;
            if (block_points[mid].first_point <= live_range.def_point)
                lo = mid + 1;
            else
                hi = mid;
        }
...
Opcode BytecodeBuilder::rk_opcode_for(IROp op) const {
    switch (op) {
        case IROp::AddI:
            return Opcode::ADD_I_RK;
        case IROp::SubI:
            return Opcode::SUB_I_RK;
        case IROp::MulI:
            return Opcode::MUL_I_RK;
        case IROp::AddF:
            return Opcode::ADD_F_RK;
        case IROp::SubF:
            return Opcode::SUB_F_RK;
        case IROp::MulF:
            return Opcode::MUL_F_RK;
        case IROp::AddD:
            return Opcode::ADD_D_RK;
        case IROp::SubD:
            return Opcode::SUB_D_RK;
        case IROp::MulD:
            return Opcode::MUL_D_RK;
        case IROp::DivD:
            return Opcode::DIV_D_RK;
        ...
        case IROp::EqD:
            return Opcode::EQ_D_RK;
        case IROp::NeD:
            return Opcode::NE_D_RK;
        case IROp::LtD:
            return Opcode::LT_D_RK;
        case IROp::LeD:
            return Opcode::LE_D_RK;
        case IROp::GtD:
            return Opcode::GT_D_RK;
        case IROp::GeD:
            return Opcode::GE_D_RK;
        default:
            return Opcode::NOP;
    }
}
...
bool BytecodeBuilder::try_emit_rk_binary(IRInst* inst, u8 dst) {
    ...
    auto def_of = [this](ValueId v) -> IRInst* {
        if (!v.is_valid() || v.id >= m_current_ir_func->values_by_id.size())
            return nullptr;
...
void BytecodeBuilder::mark_unfusable_if_cross_block(IRInst* inst, u32 cmp_pc) {
    if (!is_comparison_op(inst->op))
        return;
    if (!inst->result.is_valid() || inst->result.id >= m_value_same_block.size())
        return;
...
void BytecodeBuilder::emit(u32 instr) { m_current_func->code.push_back(instr); }

void BytecodeBuilder::emit_abc(Opcode op, u8 a, u8 b, u8 c) { emit(encode_abc(op, a, b, c)); }

void BytecodeBuilder::emit_abi(Opcode op, u8 a, u16 imm) { emit(encode_abi(op, a, imm)); }

void BytecodeBuilder::emit_aoff(Opcode op, u8 a, i16 offset) { emit(encode_aoff(op, a, offset)); }

void BytecodeBuilder::emit_call_args(Span<ValueId> args, IRFunction* callee_func, u8 first_arg_reg,
    ...
    for (u32 i = 0; i < args.size(); i++) {
        ...
        if (arg_type && arg_type->kind == TypeKind::Weak) {
            ...
            if (arg_src != arg_dst) {
                emit_abc(Opcode::MOV, arg_dst, arg_src, 0);
                emit_abc(Opcode::MOV, static_cast<u8>(arg_dst + 1), static_cast<u8>(arg_src + 1),
                         0);
            }
            arg_reg_offset += 2;
            continue;
    ...
    switch (inst->op) {
        ...
        case IROp::CallNative: {
            ...
            for (u32 i = 0; i < inst->call.args.size(); i++)
                note_call_use(inst->call.args[i]);
            break;
        }
        ...
        case IROp::CallIndirect: {
            ...
            u8 ret_reg_count = static_cast<u8>(get_value_reg_count(inst->type));
            emit_call_args(inst->call_indirect.args, nullptr, static_cast<u8>(dst + ret_reg_count),
                           true);
        ...
        case IROp::IndexGet: {
            ...
            Opcode op = (inst->index_data.kind == ContainerKind::List) ? Opcode::INDEX_GET_LIST
                                                                       : Opcode::INDEX_GET_MAP;
        ...
        case IROp::IndexAddr: {
            ...
            u8 obj_reg = ensure_in_register(inst->index_data.container, 0);
            u8 idx_reg = ensure_in_register(inst->index_data.index, 0);
            Opcode op = (inst->index_data.kind == ContainerKind::List) ? Opcode::INDEX_ADDR_LIST
                                                                       : Opcode::INDEX_ADDR_MAP;
            emit_abc(op, dst, obj_reg, idx_reg);
            spill_if_needed(inst->result, dst);
            break;
        }
        ...
        case IROp::IndexSet: {
            ...
            Opcode op = (inst->index_data.kind == ContainerKind::List) ? Opcode::INDEX_SET_LIST
                                                                       : Opcode::INDEX_SET_MAP;
        ...
        case IROp::Closure: {
            ...
            if (type_it != m_type_indices.end()) {
                ...
                if (struct_has_default_dtor(env_type)) {
                    ...
                    if (dtor_idx != 0)
                        env_info.dtor_func_idx = dtor_idx;
                }
            ...
            if (call_idx_value <= 0x7FFF) {
                ...
                u16 const_idx =
                    add_constant(BCConstant::make_int(static_cast<i64>(call_idx_value)));
            ...
            for (u32 i = 0; i < inst->closure.captures.size(); i++) {
                ...
                if (field.type && field.type->is_struct()) {
                    ...
                    switch (field.slot_count) {
                        case 1:
                            op = Opcode::STRUCT_COPY_1;
                            break;
                        case 2:
                            op = Opcode::STRUCT_COPY_2;
                            break;
                        case 3:
                            op = Opcode::STRUCT_COPY_3;
                            break;
                        case 4:
                            op = Opcode::STRUCT_COPY_4;
                            break;
                        default:
                            op = Opcode::STRUCT_COPY;
                            break;
                    }
                    emit_abc(op, dest_addr, cap_reg,
                             op == Opcode::STRUCT_COPY ? static_cast<u8>(field.slot_count) : 0);
                } else {
                    emit_abc(Opcode::SET_FIELD, dst, cap_reg, static_cast<u8>(field.slot_count));
                    emit(static_cast<u32>(field.slot_offset));
                }
            }
        ...
        case IROp::Nullify: {
            ...
            if (inst->unary.is_valid()) {
                ...
                if (kill_it != m_cleanup_kill_pcs.end()) {
                    ...
                    if (candidate != 0 && candidate < anchor)
                        anchor = candidate;
        ...
        case IROp::StructCopy: {
            ...
            emit_abc(op, dest_ptr, src_ptr,
                     op == Opcode::STRUCT_COPY ? static_cast<u8>(slot_count) : 0);
            break;
        }
...
void BytecodeBuilder::emit_block_args(const JumpTarget& target) {
    if (!target.block.is_valid() || target.block.id >= m_current_ir_func->blocks.size())
        return;
...
void BytecodeBuilder::lower_terminator(IRBlock* block) {
    ...
    switch (term.kind) {
        ...
        case TerminatorKind::Branch: {
            ...
            m_current_func->code[jmp_if_not_idx] =
                encode_aoff(Opcode::JMP_IF_NOT, cond, skip_offset);
    ...
    auto pick_fused = [](Opcode cmp_op, bool jmp_if_not) -> Opcode {
        ...
        if (cmp_op >= Opcode::EQ_I && cmp_op <= Opcode::GE_I) {
            if (jmp_if_not) {
                switch (cmp_op) {
                    case Opcode::EQ_I:
                        return Opcode::JMP_IF_NE_I;
                    case Opcode::NE_I:
                        return Opcode::JMP_IF_EQ_I;
                    case Opcode::LT_I:
                        return Opcode::JMP_IF_GE_I;
                    case Opcode::LE_I:
                        return Opcode::JMP_IF_GT_I;
                    case Opcode::GT_I:
                        return Opcode::JMP_IF_LE_I;
                    case Opcode::GE_I:
                        return Opcode::JMP_IF_LT_I;
                    default:
                        break;
                }
            } else {
        ...
        if (cmp_op >= Opcode::EQ_D && cmp_op <= Opcode::GE_D) {
            if (jmp_if_not) {
                switch (cmp_op) {
                    case Opcode::EQ_D:
                        return Opcode::JMP_IF_NE_D;
                    case Opcode::NE_D:
                        return Opcode::JMP_IF_EQ_D;
                    case Opcode::LT_D:
                        return Opcode::JMP_IF_GE_D;
                    case Opcode::LE_D:
                        return Opcode::JMP_IF_GT_D;
                    case Opcode::GT_D:
                        return Opcode::JMP_IF_LE_D;
                    case Opcode::GE_D:
                        return Opcode::JMP_IF_LT_D;
                    default:
                        break;
                }
            } else {
        ...
        if (cmp_op >= Opcode::EQ_D_RK && cmp_op <= Opcode::GE_D_RK) {
            if (jmp_if_not) {
                switch (cmp_op) {
                    case Opcode::EQ_D_RK:
                        return Opcode::JMP_IF_NE_D_RK;
                    case Opcode::NE_D_RK:
                        return Opcode::JMP_IF_EQ_D_RK;
                    case Opcode::LT_D_RK:
                        return Opcode::JMP_IF_GE_D_RK;
                    case Opcode::LE_D_RK:
                        return Opcode::JMP_IF_GT_D_RK;
                    case Opcode::GT_D_RK:
                        return Opcode::JMP_IF_LE_D_RK;
                    case Opcode::GE_D_RK:
                        return Opcode::JMP_IF_LT_D_RK;
                    default:
                        break;
                }
            } else {
...
u32 BytecodeBuilder::get_value_reg_count(Type* type) {
    if (!type)
        return 1;
    if (type->kind == TypeKind::Weak)
        return 2; // 128-bit: pointer + generation
...
Opcode BytecodeBuilder::get_opcode(IROp op) const {
    switch (op) {
        ...
        // Type conversions
        case IROp::I_TO_F64:
            return Opcode::I_TO_F64;
        case IROp::F64_TO_I:
            return Opcode::F64_TO_I;
        case IROp::I_TO_B:
            return Opcode::I_TO_B;
        case IROp::B_TO_I:
            return Opcode::B_TO_I;
...
void BytecodeBuilder::build_struct_field_deletes(Type* struct_type, u16& out_start,
                                                 u16& out_count) {
    ...
    auto append_field = [&](u32 slot_offset, Type* field_type, u32 disc_slot_offset,
                            i32 disc_value) {
            ...
            for (i32 fi = static_cast<i32>(variant.fields.size()) - 1; fi >= 0; fi--) {
                ...
                if (member_needs_drop(variant_field.type)) {
                    append_field(clause.union_slot_offset + variant_field.slot_offset,
                                 variant_field.type, clause.discriminant_slot_offset,
...
u16 BytecodeBuilder::build_delete_desc(Type* type) {
    ...
    if (cached != m_delete_desc_cache.end())
    ...
    switch (plan.kind) {
        ...
        case DropKind::WalkFields:
            ...
            build_struct_field_deletes(plan.struct_type, desc.fields.field_start,
                                       desc.fields.field_count);
            break;
        case DropKind::List:
        ...
        case DropKind::Map:
            ...
            desc.container.key_desc_idx =
                map_key_needs_drop(plan.key_type) ? build_delete_desc(plan.key_type) : 0xFFFF;
            break;
