...
bool IRValidator::validate(IRModule* module) {
    ...
    for (u32 i = 0; i < module->functions.size(); i++) {
        ...
        if (!validate_function(module->functions[i]))
            return false;
    }
...
bool IRValidator::validate_function(IRFunction* func) {
    ...
    for (u32 i = 0; i < func->params.size(); i++) {
        ...
        if (!param.value.is_valid() || param.value.id >= func->next_value_id) {
            report_error_fmt("function '{}': param[{}] has invalid ValueId {}", func->name, i,
                             param.value.id);
            return false;
        }
        if (!param.type) {
    ...
    for (u32 i = 0; i < func->blocks.size(); i++) {
        if (func->blocks[i]->id.id != i) {
            report_error_fmt("function '{}': blocks[{}]->id.id == {} (expected {})", func->name, i,
                             func->blocks[i]->id.id, i);
            return false;
        }
    }
...
bool IRValidator::validate_block(IRFunction* func, IRBlock* block) {
    ...
    for (u32 i = 0; i < block->params.size(); i++) {
        ...
        if (!param.value.is_valid() || param.value.id >= func->next_value_id) {
            report_error_fmt("function '{}' block {}: param[{}] has invalid ValueId {}", func->name,
                             block->id.id, i, param.value.id);
            return false;
        }
        if (!param.type) {
            report_error_fmt("function '{}' block {}: param[{}] has null type", func->name,
                             block->id.id, i);
            return false;
        }
    }
    ...
    for (IRInst* inst : block->instructions) {
        if (!validate_instruction(func, block, inst))
            return false;
        if (inst->op == IROp::Throw)
            has_throw = true;
    }
    ...
    if (has_throw && block->terminator.kind != TerminatorKind::Unreachable) {
        report_error_fmt(
            "function '{}' block {}: block contains Throw but terminator is not Unreachable",
            func->name, block->id.id);
        return false;
    }

    // Validate terminator
    if (!validate_terminator(func, block))
        return false;
...
static bool is_void_op(IROp op) { return op == IROp::StructCopy; }
...
bool IRValidator::validate_instruction(IRFunction* func, IRBlock* block, IRInst* inst) {
    ...
    if (inst->result.is_valid()) {
        if (inst->result.id >= next_id) {
            report_error_fmt("function '{}' block {}: instruction result ValueId {} out of range "
                             "(next_value_id={})",
        ...
        if (func->inst_for(inst->result) != inst) {
    ...
    switch (inst->op) {
        ...
        case IROp::ContainerUnpin: {
            if (!value_in_range(inst->unary, next_id)) {
                report_error_fmt("function '{}' block {}: unary op operand v{} invalid", func->name,
                                 block->id.id, inst->unary.id);
                return false;
            }
            break;
        }
        ...
        case IROp::CallNative: {
            for (u32 i = 0; i < inst->call.args.size(); i++) {
                if (!value_in_range(inst->call.args[i], next_id)) {
                    report_error_fmt("function '{}' block {}: call arg[{}] v{} invalid", func->name,
                                     block->id.id, i, inst->call.args[i].id);
                    return false;
                }
            }
            break;
        ...
        case IROp::SetField: {
            if (!value_in_range(inst->field.object, next_id)) {
                report_error_fmt("function '{}' block {}: set_field object v{} invalid", func->name,
                                 block->id.id, inst->field.object.id);
                return false;
            }
        ...
        case IROp::New: {
            for (u32 i = 0; i < inst->new_data.args.size(); i++) {
                if (!value_in_range(inst->new_data.args[i], next_id)) {
                    report_error_fmt("function '{}' block {}: new arg[{}] v{} invalid", func->name,
                                     block->id.id, i, inst->new_data.args[i].id);
                    return false;
                }
            }
            break;
        ...
        case IROp::StructCopy: {
            ...
            if (inst->struct_copy.kind == StructCopyKind::Clone && inst->struct_copy.struct_type &&
                inst->struct_copy.struct_type->noncopyable()) {
                report_error_fmt(
                    "function '{}' block {}: struct_copy marked clone but its type is move-only",
                    func->name, block->id.id);
                return false;
            }
            break;
        ...
        case IROp::LoadPtr: {
            if (!value_in_range(inst->load_ptr.ptr, next_id)) {
                report_error_fmt("function '{}' block {}: load_ptr ptr v{} invalid", func->name,
                                 block->id.id, inst->load_ptr.ptr.id);
                return false;
            }
            break;
        }
        ...
        case IROp::StorePtr: {
            if (!value_in_range(inst->store_ptr.ptr, next_id)) {
                report_error_fmt("function '{}' block {}: store_ptr ptr v{} invalid", func->name,
                                 block->id.id, inst->store_ptr.ptr.id);
                return false;
            }
            if (!value_in_range(inst->store_ptr.value, next_id)) {
                report_error_fmt("function '{}' block {}: store_ptr value v{} invalid", func->name,
                                 block->id.id, inst->store_ptr.value.id);
                return false;
            }
            break;
        }
        ...
        case IROp::Cast: {
            if (!value_in_range(inst->cast.source, next_id)) {
                report_error_fmt("function '{}' block {}: cast source v{} invalid", func->name,
                                 block->id.id, inst->cast.source.id);
                return false;
            }
            if (!inst->cast.source_type) {
                report_error_fmt("function '{}' block {}: cast has null source_type", func->name,
                                 block->id.id);
                return false;
            }
            break;
        ...
        case IROp::BlockArg: {
            if (inst->block_arg_index >= block->params.size()) {
                report_error_fmt(
                    "function '{}' block {}: block_arg_index {} >= block params size {}",
        ...
        case IROp::IndexSet: {
            ...
            if (!value_in_range(inst->index_data.index, next_id)) {
                report_error_fmt("function '{}' block {}: index op index v{} invalid", func->name,
                                 block->id.id, inst->index_data.index.id);
                return false;
            }
            if (inst->op == IROp::IndexSet && !value_in_range(inst->index_data.value, next_id)) {
                report_error_fmt("function '{}' block {}: index_set value v{} invalid", func->name,
                                 block->id.id, inst->index_data.value.id);
                return false;
            }
            break;
        }
...
bool IRValidator::validate_terminator(IRFunction* func, IRBlock* block) {
    ...
    if (term.kind == TerminatorKind::None) {
        report_error_fmt("function '{}' block {}: missing terminator", func->name, block->id.id);
        return false;
    }
    ...
    switch (term.kind) {
        ...
        case TerminatorKind::Goto: {
            if (!validate_jump_target(func, term.goto_target, "goto"))
                return false;
            break;
        }
        ...
        case TerminatorKind::Branch: {
            if (!value_in_range(term.branch.condition, func->next_value_id)) {
                report_error_fmt("function '{}' block {}: branch condition v{} invalid", func->name,
                                 block->id.id, term.branch.condition.id);
                return false;
            }
            if (!validate_jump_target(func, term.branch.then_target, "branch.then"))
                return false;
            if (!validate_jump_target(func, term.branch.else_target, "branch.else"))
                return false;
            break;
        }
...
bool IRValidator::validate_jump_target(IRFunction* func, const JumpTarget& target,
                                       const char* label) {
    if (!target.block.is_valid() || target.block.id >= func->blocks.size()) {
        report_error_fmt("function '{}': {} target block {} out of range (num blocks={})",
                         func->name, label, target.block.id, static_cast<u32>(func->blocks.size()));
        return false;
    }
    ...
    if (target.args.size() != target_block->params.size()) {
        report_error_fmt("function '{}': {} to block {}: arg count {} != target param count {}",
                         func->name, label, target.block.id, static_cast<u32>(target.args.size()),
    ...
    for (u32 i = 0; i < target.args.size(); i++) {
        if (!value_in_range(target.args[i].value, func->next_value_id)) {
            report_error_fmt("function '{}': {} to block {}: arg[{}] v{} invalid", func->name,
                             label, target.block.id, i, target.args[i].value.id);
            return false;
        }
    }
