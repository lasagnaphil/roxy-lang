...
ValueId IRBuilder::gen_unary_expr(Expr* expr) {
    ...
    if (operand_type && operand_type->is_struct()) {
        ...
        if (!method_name.empty()) {
            ...
            if (mi && found_in) {
                ValueId self_ptr = gen_lvalue_addr(unary_expr.operand, /*rvalue_ok=*/true);
    ...
    if (left_type && left_type->is_struct()) {
        ...
        if (!method_name.empty()) {
            ...
            if (mi && found_in) {
                ...
                ValueId self_ptr = gen_lvalue_addr(binary_expr.left, /*rvalue_ok=*/true);
...
ValueId IRBuilder::gen_lvalue_addr(Expr* expr, bool rvalue_ok) {
    ...
    switch (expr->kind) {
        ...
        case AstKind::ExprGrouping:
            return gen_lvalue_addr(expr->grouping.expr, rvalue_ok);
        default:
            // An operator receiver (`a + b` -> `a.add(b)`) may be an rvalue with
            // no place to address — most importantly the result of another
            // operator, which is what makes `(a + b) * 2.0f` chain. No
            // materialization is needed: lowering already unpacks a struct
